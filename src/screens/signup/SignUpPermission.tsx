...
import { ButtonWrapper, Description, Row, StyledButton } from '../auth/LastStep.styles';
import { FormWrapper, Title } from '../auth/LoginForm.styles';
import { Modal } from '../../components/modal/Modal';
import { AuthCard, Left, LeftContent, LogoWrapper, Right, RightArea, RightAreaTitle, Wrapper } from '../auth/Auth.styles';
...
const SignUpPermission: FC<LastStepProps> = ({ onSubmit, cancel }) => {
  const [isKvkkContractOpen, setIsKvkkContractOpen] = useState(false);
  const [isContractOpen, setIsContractOpen] = useState(false);
  const { control, handleSubmit, watch } = useForm();
  const showEmail = watch('isEmailActive')
                      ...
                      <Row>
                        <Controller
                          ...
                          render={({ field: { onChange, value } }) => (
                            <TextInput value={value} onChange={(event) => onChange(event)} label="Email" id="emailAdress" />
                          )}
                        />
                      </Row>
                    )
